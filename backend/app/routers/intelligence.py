...
async def get_intelligence_feed():
    """Get the latest proactively generated intelligence feed.

    This endpoint serves pre-computed intelligence items that were
    generated by the Proactive Intelligence Engine without any user query.
    The feed includes:
    - Emerging topics detected in the last cycle
    - Narrative shifts for tracked entities
    - Entity spotlights with unusual activity
    """
    ...
    return FeedResponse(
        feed=feed,
        generated_at=feed[0].get("generated_at") if feed else None,
        item_count=len(feed)
    )
...
async def generate_feed():
    ...
    Normally runs on schedule (6 AM and 6 PM), but can be triggered manually.
    """
...
async def detect_emergence():
    ...
    Normally runs every 30 minutes. Detects new topic clusters.
    """
...
async def monitor_narratives():
    ...
    Normally runs every hour. Detects sentiment shifts for tracked entities.
    """
...
async def get_trending_entities(
    ...
    Entities are ranked by their recent activity compared to baseline.
    """
...
async def get_daily_digest():
    ...
    Contains top trending topics, category breakdown, and spike alerts.
    """
    digest = await temporal_analytics.get_daily_digest()

    if not digest:
        # Trigger digest generation if not cached
...
async def get_entity_trend(
    ...
    Returns volume over time, sentiment trend, and spike detection.
    """
...
async def get_topic_trend(
    ...
    Returns volume over time and sentiment trend.
    """
...
async def get_volume_over_time(
    ...
    Can filter by entity or topic/keyword.
    """
